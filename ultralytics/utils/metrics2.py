...
def bbox_iou(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False, EIoU=False, SIoU=False, ShapeIoU=False, PIoU=False, PIoU2=False, improved=False, eps=1e-7, scale=0.0, Lambda=1.3):
    """
    ...
    if CIoU or DIoU or GIoU or EIoU or SIoU or ShapeIoU or PIoU or PIoU2:
        ...
        if CIoU or DIoU or EIoU or SIoU or PIoU or PIoU2 or ShapeIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            ...
            if CIoU:  # https://github.com/Zzh-tju/DIoU-SSD-pytorch/blob/master/utils/box/box_utils.py#L47
                ...
                # rho_y = (s_ch / ch) ** 2
                # gamma = angle_cost - 2
                # angle_distance_cost = 2 - torch.exp(gamma * rho_x) - torch.exp(gamma * rho_y)
                # omiga_w = torch.abs(w1 - w2) / torch.max(w1, w2)
                # omiga_h = torch.abs(h1 - h2) / torch.max(h1, h2)
                # shape_cost = torch.pow(1 - torch.exp(-1 * omiga_w), 4) + torch.pow(1 - torch.exp(-1 * omiga_h), 4)
                # sioU_penalty = 0.5 * (distance_cost + shape_cost)

                # sioU_value = iou - sioU_penalty + eps # 原始SIoU公式

                # PIoUv2 Component (Copied from original PIoU2 part)
                dw1 = torch.abs(b1_x2.minimum(b1_x1) - b2_x2.minimum(b2_x1))
                dw2 = torch.abs(b1_x2.maximum(b1_x1) - b2_x2.maximum(b2_x1))
                dh1 = torch.abs(b1_y2.minimum(b1_y1) - b2_y2.minimum(b2_y1))
                dh2 = torch.abs(b1_y2.maximum(b1_y1) - b2_y2.maximum(b2_y1))
                P = ((dw1 + dw2) / torch.abs(w2) + (dh1 + dh2) / torch.abs(h2)) / 4
                piou_v1 = 1 - iou - torch.exp(-P**2) + 1 + 0.02 * angle_cost
                # q = torch.exp(-P)
                # x = q * Lambda
                # piou_penalty = 3 * x * torch.exp(-x**2) * piou_v1  - 0.1 * angle_distance_cost

                # piou_v2_value = 1 - piou_penalty # 原始 PIoUv2 公式

                # Combined IoU (简单加权求和，可以尝试其他融合方式)
                return 1 - piou_v1 # 1 - 3*x*torch.exp(-x**2)*piou_v1
...
def bbox_inner_iou(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False, EIoU=False, SIoU=False, ShapeIoU=False, PIoU=False, PIoU2=True, eps=1e-7, ratio=0.7, scale=0.0, Lambda=1.3):
    """
    Calculate Intersection over Union (IoU) of box1(1, 4) to box2(n, 4).
...
def get_inner_iou_p(box1, box2, xywh=False, eps=1e-7, ratio=0.7):
...
def bbox_inner_iou_P(box1, box2, xywh=False, PIoU2=True, eps=1e-7, ratio=0.7, scale=0.0, Lambda=1.3):
    """
    Calculate Intersection over Union (IoU) of box1(1, 4) to box2(n, 4).
    ...
    # Get the coordinates of bounding boxes -- xyxy
    ...
    innner_iou = get_inner_iou_p(box1, box2, xywh=xywh, ratio=ratio)
...
def probiou(obb1, obb2, CIoU=False, eps=1e-7):
    ...
    if CIoU:  # only include the wh aspect ratio part
        ...
        w1, h1 = obb1[..., 2:4].split(1, dim=-1)  # 先搞清楚这里面的wh是什么，怎么用
